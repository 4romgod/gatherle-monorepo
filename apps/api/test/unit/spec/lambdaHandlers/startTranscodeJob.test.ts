...
describe('startTranscodeJobHandler', () => {
  ...
  describe('multiple records', () => {
    it('submits a job for each video record and skips non-video records', async () => {
      await startTranscodeJobHandler(makeEvent('a/clip.mp4'));
      await startTranscodeJobHandler(makeEvent('b/clip.mov'));
  ...
  describe('client singleton', () => {
    it('reuses the same client across multiple invocations', async () => {
      ...
      await freshHandler(makeEvent('a/clip.mp4'));
      await freshHandler(makeEvent('b/clip.mp4'));
