...
type CreateMomentProgressAware = {
  onProgressMessage?: (message: string) => void;
};

type CreateMomentArgs =
  | (CreateMomentProgressAware & {
      background?: string;
      caption: string;
      eventId: string;
      occurrenceId?: string;
...
export function useCreateEventMoment(authToken: string | null) {
  ...
  const createMoment = async (args: CreateMomentArgs) => {
    ...
    try {
      ...
      if (args.type === 'text') {
        ...
        args.onProgressMessage?.('Publishing your moment…');
      ...
      args.onProgressMessage?.('Preparing your media…');
      ...
      try {
        args.onProgressMessage?.(args.type === 'image' ? 'Uploading your image…' : 'Uploading your video…');
      ...
      if (args.type === 'video' && args.thumbnailAsset) {
        ...
        if (thumbnailUploadTarget) {
          try {
            args.onProgressMessage?.('Uploading your video preview…');
  ...
  return {
    createMoment,
    loading: isSubmitting || createState.loading || uploadState.loading,
  };
}
