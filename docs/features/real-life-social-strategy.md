...
This means Gatherle should not start as a generic event directory, and it should not try to become a full social
network on day one. The smarter path is to own the moments around going out: deciding, coordinating, showing up,
proving attendance, and remembering what happened.
...
This is one of the highest-leverage retention features because it makes Gatherle feel personal even when the user is
not planning the next event.
...
3. Add **verified attendance and host intelligence** once the product is generating enough usage to support better
   truth data.
