...
import { buildVenueInput, initialVenueFormState, validateVenueForm, type VenueFormState } from '@/lib/venues/forms';
...
export function CreateVenueScreen() {
  ...
  const { canCreateVenue, isGlobalAdmin, manageableOrganizations, loading: accessLoading } = useVenueManagementAccess();

  const [formState, setFormState] = useState<VenueFormState>(initialVenueFormState);
  ...
  useEffect(() => {
    ...
    if (manageableOrganizations.length === 1) {
      setSelectedOrgId(manageableOrganizations[0].orgId);
    }
  }, [isGlobalAdmin, manageableOrganizations, selectedOrgId]);

  const selectedOrganizationName = useMemo(
    () => manageableOrganizations.find((organization) => organization.orgId === selectedOrgId)?.name ?? null,
    [manageableOrganizations, selectedOrgId],
  );
  ...
  const submit = async () => {
    ...
    if (!isGlobalAdmin && !selectedOrgId) {
      showToast({ message: 'Choose the organization that should manage this venue.', tone: 'error' });
      return;
    }
    ...
    try {
      await withBlockingLoader('Creating venue…', async () => {
        ...
        const createdVenueName = result.data?.createVenue?.name;
        ...
        if (createdVenueId) {
          navigation.replace('VenueDetails', {
            venueId: createdVenueId,
            venueName: createdVenueName ?? formState.name.trim(),
          });
          return;
        }
      ...
      <View style={styles.section}>
        ...
        <View style={styles.choiceBlock}>
          <Text style={[styles.choiceLabel, { color: theme.colors.textPrimary }]}>
            {isGlobalAdmin ? 'Choose an organization or keep this venue standalone' : 'Choose your organization'}
          </Text>
          <View style={styles.choiceWrap}>
            ...
            {manageableOrganizations.map((organization) => (
              <AccountChoiceChip
                ...
                onPress={() => setSelectedOrgId(organization.orgId)}
                selected={selectedOrgId === organization.orgId}
              />
            ))}
          </View>
          <Text style={[styles.ownerHint, { color: theme.colors.textSecondary }]}>
            {selectedOrgId
              ? `${selectedOrganizationName ?? 'This organization'} will be able to manage this venue.`
              : isGlobalAdmin
                ? 'Standalone venues can only be managed by global admins.'
                : 'Choose the organization that should manage this venue.'}
          </Text>
        </View>
      </View>
